...
const LOGO_STYLES: { id: CampaignConfig['logoStyle']; label: string; icon: string; radius: string }[] = [
...
export default function BrandingSection({ campaign, onChange }: Props) {
  ...
  useEffect(() => {
    const isKnownPreset = GRADIENT_PRESETS.some(p => p.css === campaign.backgroundGradient);
    if (campaign.backgroundStyle === 'gradient' && !isKnownPreset) {
  ...
  }, [campaign.customGradientStart, campaign.customGradientEnd, campaign.customGradientDirection, campaign.backgroundStyle]);
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center p-6 bg-surface-container-low rounded-3xl border border-outline-variant/10">
            <div className="relative group">
              <div className={`w-24 h-24 bg-surface-container-lowest border-2 border-dashed border-outline-variant/30 flex items-center justify-center overflow-hidden transition-all duration-500 shadow-sm ${LOGO_STYLES.find(s => s.id === campaign.logoStyle)?.radius}`}>
                {campaign.logo ? (
                  <img src={campaign.logo} alt="Logo" className="w-full h-full object-contain p-2" />
                ) : (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {FONT_OPTIONS.map((font) => (
              <button
                ...
                className={`px-4 py-3.5 rounded-2xl border text-left transition-all ${
          ...
          <div className="flex gap-1 p-1 bg-surface-container-low rounded-2xl border border-outline-variant/10">
            {BG_MODES.map((mode) => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
                  campaign.backgroundStyle === mode.id
                  ...
                  {GRADIENT_PRESETS.map((preset) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative rounded-2xl overflow-hidden h-24 border-2 transition-all hover:scale-[1.03] active:scale-95 group ${
                          isSelected ? 'border-primary shadow-xl shadow-primary/10' : 'border-transparent hover:border-outline-variant/30'
                        }`}
              ...
              <div className="p-6 bg-surface-container-low rounded-3xl border border-outline-variant/10 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-outline-variant/10 shadow-sm">
                      ...
                      />
                      <span className="text-[10px] font-mono font-bold uppercase text-on-surface tracking-wider">{campaign.customGradientStart}</span>
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-outline-variant/10 shadow-sm">
                      <input 
                        ...
                        className="w-7 h-7 rounded-lg cursor-pointer border-none p-0 bg-transparent"
                      />
                      <span className="text-[10px] font-mono font-bold uppercase text-on-surface tracking-wider">{campaign.customGradientEnd}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/60">Angle: {campaign.customGradientDirection}</label>
                  </div>
                  <input 
