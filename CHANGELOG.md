...
### Fixed
- **Backend (CORS)**: Configured dynamic CORS allowed origins dynamically loaded from `Frontend:AllowedOrigins` (falling back to `Frontend:BaseUrl`) in [Program.cs](file:///home/ducdat/IT/CNPM/LT-Web-ASP.Net-Core/reviewloom/backend/src/ReviewLoom.Api/Program.cs). Added the deployed Vercel origin (`https://reviewloom.vercel.app`) to `AllowedOrigins` in [appsettings.json](file:///home/ducdat/IT/CNPM/LT-Web-ASP.Net-Core/reviewloom/backend/src/ReviewLoom.Api/appsettings.json) and [appsettings.Development.json](file:///home/ducdat/IT/CNPM/LT-Web-ASP.Net-Core/reviewloom/backend/src/ReviewLoom.Api/appsettings.Development.json) to resolve preflight blocking issues during production API calls.
